...
static void write_runtime_scripts(const hase_config_t *cfg) {
    ...
    write_file(path,
        ...
        "    if run_step \"$label (try $attempt/$tries)\" \"$log\" \"$@\"; then\n"
        "      return 0\n"
        "    fi\n"
        "    if [ \"$attempt\" -lt \"$tries\" ]; then\n"
        "      delay=$((attempt * 5))\n"
        "      note \"Retrying $label in ${delay}s.\"\n"
        "      sleep \"$delay\"\n"
        "    fi\n"
        "    attempt=$((attempt + 1))\n"
        "  done\n"
        ...
        "  fi\n"
        "  retry_step 'Refreshing FEX package index' \"$LOGDIR/fex-apt-update.log\" 6 refresh_fex_index\n"
