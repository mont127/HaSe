...
static void write_runtime_scripts(const hase_config_t *cfg) {
    ...
    write_file(path,
        ...
        "GEOMETRY=\"${HASE_GEOMETRY:-640x360x24}\"\n"
        "FBDIR=\"${HASE_XVFB_FBDIR:-/mnt/hase/runtime/fb}\"\n"
        "mkdir -p /tmp/hase\n"
        "if [ -f /tmp/hase/geometry ] && [ \"$(cat /tmp/hase/geometry)\" != \"$GEOMETRY\" ]; then\n"
        "  pkill -f \"Xvfb ${DISPLAY}\" >/dev/null 2>&1 || true\n"
        "fi\n"
        "if [ -f /tmp/hase/fbdir ] && [ \"$(cat /tmp/hase/fbdir)\" != \"$FBDIR\" ]; then\n"
        "  pkill -f \"Xvfb ${DISPLAY}\" >/dev/null 2>&1 || true\n"
        "fi\n"
        "if [ ! -f /tmp/hase/fbdir ] && pgrep -f \"Xvfb ${DISPLAY}\" >/dev/null; then\n"
        "  pkill -f \"Xvfb ${DISPLAY}\" >/dev/null 2>&1 || true\n"
        "fi\n"
        ...
        "  mkdir -p \"$FBDIR\"\n"
        "  rm -f \"$FBDIR/Xvfb_screen0\"\n"
        "  Xvfb \"${DISPLAY}\" -screen 0 \"$GEOMETRY\" -fbdir \"$FBDIR\" -nolisten tcp >/tmp/hase/xvfb.log 2>&1 &\n"
        "  printf '%s\\n' \"$GEOMETRY\" >/tmp/hase/geometry\n"
        "  printf '%s\\n' \"$FBDIR\" >/tmp/hase/fbdir\n"
        "  sleep 1\n"
        "fi\n"
    ...
    write_file(path,
        ...
        "TARGET_FPS=\"${HASE_CAPTURE_TARGET_FPS:-30}\"\n"
        "MIN_FPS=\"${HASE_CAPTURE_MIN_FPS:-20}\"\n"
        ...
        "LIVE_FB=\"${HASE_XVFB_FBDIR:-/mnt/hase/runtime/fb}/Xvfb_screen0\"\n"
        "mkdir -p /mnt/hase/runtime\n"
        "if [ -f \"$LIVE_FB\" ] && [ \"${HASE_CAPTURE_LEGACY_XWD:-0}\" != 1 ]; then\n"
        "  printf 'Using live Xvfb framebuffer at %s; capture daemon is idle.\\n' \"$LIVE_FB\"\n"
        ...
        "  : > \"$current\"\n"
        "  windows=/tmp/hase/capture-windows.tsv\n"
        "  DISPLAY=\"$DISPLAY\" wmctrl -lG -p 2>/dev/null | awk '\n"
        "    BEGIN { OFS=\"\\t\" }\n"
        "    { title=\"\"; for (i=9; i<=NF; ++i) title=title (i==9 ? \"\" : \" \") $i; low=tolower(title); is_game=(low !~ /(steam|friends|sign in|settings)/); if ($6 >= 80 && $7 >= 80) print $1, is_game, title }\n"
        "  ' > \"$windows\"\n"
        "  has_game=0\n"
        "  if awk -F '\\t' '$2 == 1 { found=1 } END { exit found ? 0 : 1 }' \"$windows\"; then has_game=1; fi\n"
        "  captured=0\n"
        "  while IFS='	' read -r id is_game title; do\n"
        "    case \"$id\" in 0x*) ;; *) continue ;; esac\n"
        "    if [ \"$has_game\" = 1 ] && [ \"$is_game\" != 1 ] && [ \"${HASE_CAPTURE_STEAM_WHILE_GAME:-0}\" != 1 ]; then continue; fi\n"
        "    printf '%s\\n' \"$id\" >> \"$current\"\n"
        ...
        "while true; do\n"
        "  captured_windows=0\n"
        "  if capture_window_frames; then captured_windows=1; fi\n"
        "  if [ \"${HASE_CAPTURE_ROOT_FRAMES:-0}\" = 1 ] || [ \"$captured_windows\" != 1 ]; then\n"
        "  if [ \"$FORMAT\" = bmp ]; then\n"
    ...
    write_file(path,
        ...
        "EOF\n"
        "mkdir -p \"$HOME/.config/pulse\" \"$HOME/.pulse\"\n"
        "cp \"$CONF\" \"$HOME/.config/pulse/client.conf\" 2>/dev/null || true\n"
        "cp \"$CONF\" \"$HOME/.pulse/client.conf\" 2>/dev/null || true\n"
        "export PULSE_SERVER=\"$SERVER\"\n"
        "export PULSE_CLIENTCONFIG=\"$CONF\"\n"
    ...
    write_file(path,
        ...
        "(cd \"$SRCDIR\" && tar -xf -)\n"
        "MISSING=\"\"\n"
        "for tool in cmake gcc make glslangValidator x86_64-linux-gnu-gcc x86_64-w64-mingw32-gcc; do\n"
        ...
        "    glslang-tools gcc-x86-64-linux-gnu libc6-dev-amd64-cross \\\n"
        "    mingw-w64 >/dev/null\n"
        "fi\n"
        "NATIVE_BUILD=\"$BUILDDIR/native\"\n"
        "X64_BUILD=\"$BUILDDIR/x86_64\"\n"
        "cmake -S \"$SRCDIR\" -B \"$NATIVE_BUILD\" \\\n"
        ...
        "    exit 1\n"
        "  }\n"
        "cmake --build \"$NATIVE_BUILD\" -j\"$(nproc)\" >/tmp/cb-icd-build.log 2>&1 || {\n"
        "    echo 'native ICD build failed; see /tmp/cb-icd-build.log' >&2\n"
        "    tail -n 30 /tmp/cb-icd-build.log >&2\n"
        "    exit 1\n"
        "  }\n"
        "cmake -S \"$SRCDIR\" -B \"$X64_BUILD\" \\\n"
        "  -DCMAKE_SYSTEM_NAME=Linux -DCMAKE_SYSTEM_PROCESSOR=x86_64 \\\n"
        "  -DCMAKE_C_COMPILER=x86_64-linux-gnu-gcc \\\n"
        "  -DCMAKE_TRY_COMPILE_TARGET_TYPE=STATIC_LIBRARY \\\n"
        "  -DCHEESEBRIDGE_BUILD_GUEST=ON -DCHEESEBRIDGE_BUILD_DEMO=OFF \\\n"
        "  -DHASE_BUILD_MANAGER=OFF -DCHEESEBRIDGE_GUEST_HEADERS_ONLY=ON \\\n"
        "  >/tmp/cb-icd-x86_64-cmake.log 2>&1 || {\n"
        "    echo 'x86_64 ICD configure failed; see /tmp/cb-icd-x86_64-cmake.log' >&2\n"
        "    tail -n 40 /tmp/cb-icd-x86_64-cmake.log >&2\n"
        "    exit 1\n"
        "  }\n"
        "cmake --build \"$X64_BUILD\" --target cheesebridge_icd -j\"$(nproc)\" \\\n"
        "  >/tmp/cb-icd-x86_64-build.log 2>&1 || {\n"
        "    echo 'x86_64 ICD build failed; see /tmp/cb-icd-x86_64-build.log' >&2\n"
        "    tail -n 40 /tmp/cb-icd-x86_64-build.log >&2\n"
        "    exit 1\n"
        "  }\n"
        "DST=/mnt/hase/vulkan/icd.d\n"
        "mkdir -p \"$DST\"\n"
        "install -m 0755 \"$NATIVE_BUILD/guest/libCheeseBridge_icd.so\" \\\n"
        "  \"$DST/libCheeseBridge_icd.so\"\n"
        "install -m 0755 \"$X64_BUILD/guest/libCheeseBridge_icd.so\" \\\n"
        "  \"$DST/libCheeseBridge_icd.x86_64.so\"\n"
        ...
        "}\n"
        "JSON\n"
        "cat > \"$DST/cheesebridge_icd.x86_64.json\" <<JSON\n"
        "{\n"
        "    \"file_format_version\": \"1.0.1\",\n"
        "    \"ICD\": {\n"
        "        \"library_path\": \"$DST/libCheeseBridge_icd.x86_64.so\",\n"
        "        \"api_version\": \"1.0.0\"\n"
        "    }\n"
        "}\n"
        "JSON\n"
        "if [ -x \"$NATIVE_BUILD/demo/cheesebridge_clear_demo\" ]; then\n"
        "  install -m 0755 \"$NATIVE_BUILD/demo/cheesebridge_clear_demo\" \"$DST/clear-demo\"\n"
        "fi\n"
        "if [ -x \"$NATIVE_BUILD/demo/cheesebridge_triangle_demo\" ]; then\n"
        "  install -m 0755 \"$NATIVE_BUILD/demo/cheesebridge_triangle_demo\" \"$DST/triangle-demo\"\n"
        "fi\n"
        ...
        "    }\n"
        "fi\n"
        "echo \"installed native ICD: $DST/libCheeseBridge_icd.so\"\n"
        "echo \"installed x86_64 ICD: $DST/libCheeseBridge_icd.x86_64.so\"\n"
        "echo \"native manifest:    $DST/cheesebridge_icd.json\"\n"
        "echo \"Proton manifest:    $DST/cheesebridge_icd.x86_64.json\"\n",
        0755);
    ...
    write_file(path,
        ...
        "# Steam itself stays on Lavapipe. Proton games are wrapped automatically;\n"
        "# this remains useful as a manual per-game fallback for native titles.\n"
        "set -eu\n"
        "if [ \"$#\" -lt 1 ]; then\n"
        ...
        "  exit 2\n"
        "fi\n"
        ". /mnt/hase/runtime/cheesebridge-game-env.sh\n"
    ...
    path_join(path, sizeof path, runtime, "cheesebridge-game-env.sh");
    write_file(path,
        "#!/bin/sh\n"
        "# Shared game-process environment for CheeseBridge Vulkan routing.\n"
        "hase_cb_env_fail() {\n"
        "  echo \"$1\" >&2\n"
        "  return 1 2>/dev/null || exit 1\n"
        "}\n"
        "DST=/mnt/hase/vulkan/icd.d\n"
        "case \"${HASE_CHEESEBRIDGE_ICD_ARCH:-x86_64}\" in\n"
        "  native|aarch64|arm64)\n"
        "    MANIFEST=\"$DST/cheesebridge_icd.json\"\n"
        "    ICD_SO=\"$DST/libCheeseBridge_icd.so\"\n"
        "    ;;\n"
        "  x86_64|amd64)\n"
        "    MANIFEST=\"$DST/cheesebridge_icd.x86_64.json\"\n"
        "    ICD_SO=\"$DST/libCheeseBridge_icd.x86_64.so\"\n"
        "    ;;\n"
        "  *)\n"
        "    hase_cb_env_fail \"Unknown HASE_CHEESEBRIDGE_ICD_ARCH=${HASE_CHEESEBRIDGE_ICD_ARCH}\"\n"
        "    ;;\n"
        "esac\n"
        ...
        "export CHEESEBRIDGE_ICD_MANIFEST=\"$MANIFEST\"\n"
        "export VK_DRIVER_FILES=\"$MANIFEST\"\n"
        "export VK_ICD_FILENAMES=\"$MANIFEST\"\n"
        "hase_append_path_var() {\n"
        "  name=\"$1\"\n"
        "  value=\"$2\"\n"
        "  eval current=\\\"\\${$name:-}\\\"\n"
        "  case \":$current:\" in *\":$value:\"*) return 0 ;; esac\n"
        "  if [ -n \"$current\" ]; then export \"$name=$current:$value\"; else export \"$name=$value\"; fi\n"
        "}\n"
        "hase_append_path_var STEAM_COMPAT_MOUNTS /mnt/hase\n"
        "hase_append_path_var STEAM_COMPAT_MOUNTS /mnt/hase/vulkan\n"
        "hase_append_path_var STEAM_COMPAT_MOUNTS /mnt/hase/vulkan/icd.d\n"
        "hase_append_path_var PRESSURE_VESSEL_FILESYSTEMS_RW /mnt/hase\n"
        "hase_append_path_var PRESSURE_VESSEL_FILESYSTEMS_RW /mnt/hase/vulkan\n"
        "hase_append_path_var PRESSURE_VESSEL_FILESYSTEMS_RW /mnt/hase/vulkan/icd.d\n"
        "hase_append_path_var PRESSURE_VESSEL_FILESYSTEMS_RO /mnt/hase\n"
        "hase_append_path_var PRESSURE_VESSEL_FILESYSTEMS_RO /mnt/hase/vulkan\n"
        "hase_append_path_var PRESSURE_VESSEL_FILESYSTEMS_RO /mnt/hase/vulkan/icd.d\n"
        ...
        "if [ -n \"${HASE_PROTON_LOG:-}\" ]; then export PROTON_LOG=\"$HASE_PROTON_LOG\"; else unset PROTON_LOG; fi\n"
        "if [ \"${HASE_DISABLE_STEAM_GAME_OVERLAY:-1}\" = 1 ]; then\n"
    ...
    path_join(path, sizeof path, runtime, "proton-bridge-daemon.sh");
    write_file(path,
        "#!/bin/sh\n"
        "# Keep installed Steam Proton runtimes wrapped so games launched from\n"
        "# Steam automatically use CheeseBridge for DXVK/VKD3D Vulkan calls.\n"
        "set -eu\n"
        "LOG=/tmp/hase/proton-bridge.log\n"
        "INTERVAL=\"${HASE_PROTON_BRIDGE_INTERVAL:-10}\"\n"
        "mkdir -p /tmp/hase\n"
        "log() { printf '[proton-bridge] %s %s\\n' \"$(date '+%Y-%m-%d %H:%M:%S')\" \"$*\" >>\"$LOG\"; }\n"
        "write_proton_wrapper() {\n"
        "  target=\"$1\"\n"
        "  case \"$target\" in */HaSe-CheeseBridge-Proton/*) return 0 ;; esac\n"
        "  [ -f \"$target\" ] || return 0\n"
        "  if grep -q 'HaSe CheeseBridge Proton wrapper' \"$target\" 2>/dev/null; then return 0; fi\n"
        "  real=\"$target.hase-real\"\n"
        "  if [ -e \"$real\" ]; then\n"
        "    saved=\"$target.hase-replaced-$(date +%s)\"\n"
        "    mv \"$target\" \"$saved\" 2>/dev/null || true\n"
        "    chmod +x \"$real\" 2>/dev/null || true\n"
        "    log \"reinstalled wrapper for $target using existing backup $real\"\n"
        "  else\n"
        "    mv \"$target\" \"$real\"\n"
        "    chmod +x \"$real\" 2>/dev/null || true\n"
        "  fi\n"
        "  cat >\"$target\" <<'WRAP'\n"
        "#!/bin/sh\n"
        "# HaSe CheeseBridge Proton wrapper\n"
        "set -eu\n"
        "REAL=\"$0.hase-real\"\n"
        "if [ ! -x \"$REAL\" ]; then\n"
        "  echo \"HaSe Proton wrapper missing real Proton: $REAL\" >&2\n"
        "  exit 1\n"
        "fi\n"
        ". /mnt/hase/runtime/cheesebridge-game-env.sh\n"
        "printf '[proton-wrapper] %s %s %s\\n' \"$(date '+%Y-%m-%d %H:%M:%S')\" \"$REAL\" \"$*\" >>/tmp/hase/proton-bridge.log\n"
        "exec \"$REAL\" \"$@\"\n"
        "WRAP\n"
        "  chmod +x \"$target\"\n"
        "  log \"wrapped $target\"\n"
        "}\n"
        "install_compat_tool() {\n"
        "  for base in \"$HOME/.local/share/Steam\" \"$HOME/.steam/root\"; do\n"
        "    [ -d \"$base\" ] || continue\n"
        "    tool=\"$base/compatibilitytools.d/HaSe-CheeseBridge-Proton\"\n"
        "    mkdir -p \"$tool\"\n"
        "    cat >\"$tool/compatibilitytool.vdf\" <<'VDF'\n"
        "\"compatibilitytools\"\n"
        "{\n"
        "  \"compat_tools\"\n"
        "  {\n"
        "    \"HaSe-CheeseBridge-Proton\"\n"
        "    {\n"
        "      \"install_path\" \".\"\n"
        "      \"display_name\" \"HaSe CheeseBridge Proton\"\n"
        "      \"from_oslist\" \"windows\"\n"
        "      \"to_oslist\" \"linux\"\n"
        "    }\n"
        "  }\n"
        "}\n"
        "VDF\n"
        "    cat >\"$tool/proton\" <<'PROTON'\n"
        "#!/bin/sh\n"
        "# HaSe CheeseBridge compatibility-tool Proton wrapper\n"
        "set -eu\n"
        "find_real_proton() {\n"
        "  for common in \"$HOME/.local/share/Steam/steamapps/common\" \"$HOME/.steam/root/steamapps/common\"; do\n"
        "    [ -d \"$common\" ] || continue\n"
        "    find \"$common\" -maxdepth 4 -type f \\( -name proton.hase-real -o -name proton \\) -print 2>/dev/null | while IFS= read -r p; do\n"
        "      case \"$p\" in */HaSe-CheeseBridge-Proton/*) continue ;; esac\n"
        "      if [ -x \"$p\" ]; then printf '%s\\n' \"$p\"; exit 0; fi\n"
        "    done\n"
        "  done\n"
        "}\n"
        "REAL=\"${HASE_REAL_PROTON:-$(find_real_proton | head -n1)}\"\n"
        "[ -n \"$REAL\" ] || { echo 'No real Proton installation found for HaSe CheeseBridge wrapper.' >&2; exit 1; }\n"
        ". /mnt/hase/runtime/cheesebridge-game-env.sh\n"
        "printf '[compat-tool] %s %s %s\\n' \"$(date '+%Y-%m-%d %H:%M:%S')\" \"$REAL\" \"$*\" >>/tmp/hase/proton-bridge.log\n"
        "exec \"$REAL\" \"$@\"\n"
        "PROTON\n"
        "    chmod +x \"$tool/proton\"\n"
        "  done\n"
        ...
        "  for common in \"$HOME/.local/share/Steam/steamapps/common\" \"$HOME/.steam/root/steamapps/common\"; do\n"
        "    [ -d \"$common\" ] || continue\n"
        "    find \"$common\" -maxdepth 4 -type f -name proton -print 2>/dev/null | while IFS= read -r proton; do\n"
        "      write_proton_wrapper \"$proton\"\n"
        "    done\n"
        "  done\n"
        "}\n"
        "if [ \"${1:-}\" = --once ]; then\n"
    ...
    write_file(path,
        ...
        "export CHEESEBRIDGE_HOST_ICD=\"/mnt/hase/vulkan/icd.d/cheesebridge_icd.x86_64.json\"\n"
        "export CHEESEBRIDGE_ICD_PATH=\"/mnt/hase/vulkan/icd.d/libCheeseBridge_icd.x86_64.so\"\n"
        "if [ \"${HASE_AUTO_PROTON_BRIDGE:-1}\" = 1 ]; then\n"
        "  /mnt/hase/runtime/proton-bridge-daemon.sh --once >/tmp/hase/proton-bridge-once.log 2>&1 || true\n"
        "  PROTON_BRIDGE_PID=/tmp/hase/proton-bridge.pid\n"
        "  if [ ! -s \"$PROTON_BRIDGE_PID\" ] || ! kill -0 \"$(cat \"$PROTON_BRIDGE_PID\")\" >/dev/null 2>&1; then\n"
        "    nohup /mnt/hase/runtime/proton-bridge-daemon.sh >/tmp/hase/proton-bridge-daemon.out 2>&1 &\n"
        "    echo \"$!\" >\"$PROTON_BRIDGE_PID\"\n"
        "  fi\n"
        "fi\n"
        "steam_running() {\n"
        ...
        "  pkill -f '[c]apture-daemon.sh' >/dev/null 2>&1 || true\n"
        "  pkill -f '[i]nput-daemon.sh' >/dev/null 2>&1 || true\n"
        "  HASE_CAPTURE_FORMAT=\"${HASE_CAPTURE_FORMAT:-xwd}\" \\\n"
        "  HASE_CAPTURE_TARGET_FPS=\"${HASE_CAPTURE_TARGET_FPS:-30}\" \\\n"
        "  HASE_CAPTURE_MIN_FPS=\"${HASE_CAPTURE_MIN_FPS:-20}\" \\\n"
        "  HASE_CAPTURE_ROOT_FRAMES=\"${HASE_CAPTURE_ROOT_FRAMES:-0}\" \\\n"
        "  HASE_CAPTURE_STEAM_WHILE_GAME=\"${HASE_CAPTURE_STEAM_WHILE_GAME:-0}\" \\\n"
        "    nohup /mnt/hase/runtime/capture-daemon.sh >/tmp/hase/capture.log 2>&1 &\n"
        "  HASE_INPUT_DELAY=\"${HASE_INPUT_DELAY:-0.003}\" \\\n"
...
static int cmd_game_bridge(const hase_config_t *cfg, const char *argv0) {
    ...
    char *argv[] = {
        ...
        "  echo \"$!\" >\"$PROTON_BRIDGE_PID\"; "
        "fi; "
        "printf 'CheeseBridge Proton bridge is active for Steam-launched Windows games.\\n'; "
        "printf 'Native-game fallback launch option:\\n'; "
        "cat /mnt/hase/runtime/cheesebridge-game-launch-option.txt; "
        "if [ -f /mnt/hase/vulkan/icd.d/cheesebridge_icd.x86_64.json ]; then "
        "  printf 'Proton ICD: installed at /mnt/hase/vulkan/icd.d/cheesebridge_icd.x86_64.json\\n'; "
        "elif [ -f /mnt/hase/vulkan/icd.d/cheesebridge_icd.json ]; then "
        "  printf 'Native ICD only: run hasectl install-icd <bottle> again to build the x86_64 Proton ICD.\\n'; "
        "else "
        ...
        "printf 'Proton wrapper log: /tmp/hase/proton-bridge.log\\n'; "
