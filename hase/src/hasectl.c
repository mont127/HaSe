...
static void write_runtime_scripts(const hase_config_t *cfg) {
    ...
    write_file(path,
        ...
        "  echo 'installing FEX packages inside HaSe VM...' >&2\n"
        "  sudo apt-get update -qq >/dev/null\n"
        "  sudo DEBIAN_FRONTEND=noninteractive apt-get install -y -qq \\\n"
        "    ca-certificates curl software-properties-common fuse3 \\\n"
        "    squashfs-tools squashfuse >/dev/null\n"
        "  if ! apt-cache policy | grep -q '/fex-emu/fex/ubuntu'; then\n"
        "    sudo add-apt-repository -y ppa:fex-emu/fex >/dev/null\n"
        "    sudo apt-get update -qq >/dev/null\n"
        "  fi\n"
        ...
        "    fi\n"
        "  done\n"
        "  sudo DEBIAN_FRONTEND=noninteractive apt-get install -y -qq $FEX_PACKAGES >/dev/null\n"
        "fi\n"
        "if ! command -v FEXBash >/dev/null 2>&1 || ! command -v FEXRootFSFetcher >/dev/null 2>&1; then\n"
        ...
        "  echo 'fetching FEX Ubuntu 24.04 x86_64 rootfs...' >&2\n"
        "  FEXRootFSFetcher -y -a --distro-name=ubuntu --distro-version=24.04 || FEXRootFSFetcher -y -a\n"
        "fi\n"
        "write_env\n"
        ". \"$ENV\"\n"
        "if ! FEXBash -c 'uname -m' >/tmp/hase-fex-test.log 2>&1; then\n"
        "  echo 'FEX installed, but a basic FEXBash test failed.' >&2\n"
        "  cat /tmp/hase-fex-test.log >&2 || true\n"
        "  exit 1\n"
        "fi\n"
        "echo \"FEX ready: $ROOT\"\n",
        0755);
