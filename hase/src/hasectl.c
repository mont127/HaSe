...
typedef struct hase_config {
    ...
    bool arch_set;
    /* "aarch64" (default) or "x86_64". x86_64 on Apple Silicon enables
     * Rosetta translation when available so Steam/Wine run at near-native
     * speed without QEMU's slow software emulation. */
    char arch[16];
} hase_config_t;
...
static void usage(FILE *out) {
    fprintf(out,
        ...
        "\n"
        "Options:\n"
        "  --arch aarch64|x86_64  VM architecture for generated Lima config.\n"
        "                         Default: aarch64. x86_64 enables Lima Rosetta.\n"
        "\n"
...
static void load_existing_arch(hase_config_t *cfg) {
    char metadata[PATH_MAX], path[PATH_MAX];
    ...
    fclose(f);
    buf[n] = '\0';
    if (strstr(buf, "\"arch\": \"x86_64\"")) {
        snprintf(cfg->arch, sizeof cfg->arch, "%s", "x86_64");
    } else if (strstr(buf, "\"arch\": \"aarch64\"")) {
        snprintf(cfg->arch, sizeof cfg->arch, "%s", "aarch64");
    }
}
...
static void parse_common(int argc, char **argv, hase_config_t *cfg) {
    ...
    for (int i = 3; i < argc; ++i) {
        if (!strcmp(argv[i], "--root")) {
            ...
            if (++i >= argc) die("--arch requires aarch64 or x86_64");
            if (strcmp(argv[i], "aarch64") && strcmp(argv[i], "x86_64"))
                die("invalid --arch '%s' (use aarch64 or x86_64)", argv[i]);
            snprintf(cfg->arch, sizeof cfg->arch, "%s", argv[i]);
            cfg->arch_set = true;
        } else {
    ...
    if (!cfg->arch_set) load_existing_arch(cfg);
...
static void write_lima_yaml(const hase_config_t *cfg) {
    ...
    const bool x86_64 = !strcmp(cfg->arch, "x86_64");
    const char *image = x86_64
...
static void write_runtime_scripts(const hase_config_t *cfg) {
    ...
    write_file(path,
        ...
        "    echo 'installing Wine tools inside x86_64 HaSe VM...' >&2\n"
        "    sudo apt-get update -qq >/dev/null\n"
        "    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y -qq \\\n"
        "      wine64 wine winetricks cabextract unzip p7zip-full >/dev/null\n"
        "  else\n"
        "    echo 'x86_64 Wine is not available.' >&2\n"
        "    echo 'For the default ARM64 HaSe VM, install FEX plus an x86_64 Wine rootfs,' >&2\n"
        "    echo 'or create an x86_64 Lima bottle for this smoke test.' >&2\n"
        "    exit 2\n"
        "  fi\n"
        "fi\n"
        "run_x86 'wineboot -u'\n"
    ...
    write_file(path,
        ...
        "if command -v FEXBash >/dev/null 2>&1; then\n"
        "  exec FEXBash -c 'if command -v wine64 >/dev/null 2>&1; then wine64 \"$HASE_DXVK_SMOKE_EXE\"; else wine \"$HASE_DXVK_SMOKE_EXE\"; fi'\n"
        "fi\n"
        ...
        "  exec wine \"$EXE\"\n"
        "fi\n"
        "echo 'Wine is not available; run hasectl prepare-dxvk-smoke <bottle> first.' >&2\n"
        "exit 2\n",
        0755);
...
static void write_metadata(const hase_config_t *cfg) {
    ...
    appendf(&json, &cap, &pos,
        ...
        "  \"default_display\": \":99\"\n"
        "}\n",
        cfg->name, cfg->vm_name, cfg->arch, bottle, lima);
